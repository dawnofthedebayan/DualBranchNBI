...
def parse_option():
    ...
    opt.model_path = './save/CEScatter/{}_models'.format(opt.dataset)
    opt.tb_path = './save/CEScatter/{}_tensorboard'.format(opt.dataset)
    ...
    opt.model_name = 'CEScatter_{}_{}_modelnumber_{}_aug_{}'.\
...
def set_model(opt):
    model_scatter = Scattering2dResNet(opt.K, opt.width)
    model = SupCEResNet(name=opt.model, num_classes=opt.n_cls)
    classifier = LinearClassifier(name=opt.model, num_classes=opt.n_cls,extra_dim=512)
...
def train(train_loader, model, model_scatter, classifier, criterion, optimizer, epoch, opt):
    ...
    for idx, (images, labels) in enumerate(train_loader):
        ...
        features_ce_scat = torch.cat((features_ce,f_scat_encoder),1)
