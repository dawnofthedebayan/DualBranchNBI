...
from util import set_optimizer, save_model, seed_everything, make_weights_for_balanced_classes
...
def parse_option():
    ...
    opt.model_path = './save/SupConBasic/{}_models'.format(opt.dataset)
    opt.tb_path = './save/SupConBasic/{}_tensorboard'.format(opt.dataset)
    ...
    opt.model_name = 'SupConBasic_{}_{}_modelnumber_{}_aug_{}'.\
...
def set_model(opt):
    ...
    return model, classifier, criterion_supcon, criterion_ce


def train(train_loader, model, classifier, criterion_supcon, criterion_ce, optimizer_model,optimizer_classifier, epoch, opt):
    """one epoch training"""
    model.train()
    classifier.train()
    ...
    for idx, (images, labels) in enumerate(train_loader):
        ...
        output  = classifier(features_ce.detach())
...
def main():
    ...
    model, classifier, criterion_supcon,criterion_ce = set_model(opt)
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        # train for one epoch
        time1 = time.time()
        loss_supcon,loss_ce  = train(train_loader, model,classifier, criterion_supcon, criterion_ce, optimizer_model,optimizer_classifier, epoch, opt)
        time2 = time.time()
        ...
        # evaluation
        loss, val_acc = validate(val_loader, model, classifier, criterion_ce, opt)
